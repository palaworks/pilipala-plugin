    ...
    { new grpc_code_gen.comment.CommentService.CommentServiceBase() with
        ...
        override self.GetOne(req: get_one.Req, ctx: Ctx) =
            ...
            | Some user ->
                ...
                <| fun e -> grpc_code_gen.comment.get_one.Rsp(Ok = false, Msg = e.Message)
        ...
        override self.GetSome(req: get_some.Req, ctx: Ctx) =
            ...
            | Some user ->
                ...
                <| fun msg -> grpc_code_gen.comment.get_some.Rsp(Ok = false, Msg = msg)
            | None -> grpc_code_gen.comment.get_some.Rsp(Ok = false, Msg = token_check_failed req.Token)
            |> Task.FromResult
